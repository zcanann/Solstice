--[[
    ...
    The folders in the "MineableItems" folder contain the properties of each mineable item. For example. the folders named "COPPER_ORE" contain the information for each
    mienable copper ore. Each of these folders contain a "MinedAsset" and "UnminedAsset" property. These properties determine what the mineable item should look like
    after it has been mined and what it should like when unmined.
    ...
    If you are ever confused by what the custom properties do, try reading the tooltips for each custom property.
    ...
    TEAM META INVENTORY INTEGRATION

    To integrate the mining system with Team META's inventory system you must first drop the "ItemSystems_Database" script into 
    the "Database" property of the "MiningResourceHandler" script. Then, all of the mineable items must have resource names
    that correspond to item names. Currently only the trees work with the Team META Inventory system because they give players 
    the "Wood" resource which corresponds with the "Wood" item in Team META's inventory system. For any more questions, 
    you can contact me on Discord at FleshyOverlord#5634
]]--