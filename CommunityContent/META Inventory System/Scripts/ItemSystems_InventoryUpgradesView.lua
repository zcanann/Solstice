...
function view:SetupDragDropHitbox()
    ...
    -- Hacky, but this will do for now.
    local TempCrossView = function() return self:IsCursorOverHitbox() end
    LOCAL_PLAYER.clientUserData.upgradeDragDropBox = TempCrossView
end
...
function view:BeginInventoryExternalInteraction()
    ...
    LOCAL_PLAYER.clientUserData.upgradeViewOpen = true
    -- If the player is currently looting a container then force close the container.
    if LOCAL_PLAYER.clientUserData.containerInventory then
        Events.Broadcast("ForceCloseViewByName","ContainerView")
...
function view:EndInventoryExternalInteraction()
    if INVENTORY_VIEW.clientUserData.externalInteractionTarget == self then
        ...
        LOCAL_PLAYER.clientUserData.upgradeViewOpen = false
    end
end
...
function view:UPDATE_AwaitingUpgradeConfirmation(dt)
    ...
    if self.elapsed == 0 then
        ...
        self.selectedPrimaryItem = inventory:GetItem(self.selectedPrimaryItemSlotIndex)  
...
inventory.OnInventoryChanged:Connect(function()
    view:MakePrimaryItemSelection(0)
end)

Events.Connect("SelectSlotForUpgrade",function(inventorySlotIndex) 
    view:MakePrimaryItemSelection(inventorySlotIndex)
end)