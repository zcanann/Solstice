...
function view:InitItemHover()
    ...
    if not PANEL_ITEM_HOVER.clientUserData.innerBaseHeight then
        PANEL_ITEM_HOVER.clientUserData.innerBaseHeight = PANEL_ITEM_HOVER.clientUserData.inner.height
    else
        PANEL_ITEM_HOVER.clientUserData.inner.height = PANEL_ITEM_HOVER.clientUserData.inner.height
    end
...
function view:DrawHoverInfo()
    if self.isCursorInContainer and self.itemUnderCursor and not self.isDragging then
        ...
        if compareEnhancementLevel and compareEnhancementLevel < item:GetEnhancementLevel() then
            PANEL_ITEM_HOVER.clientUserData.enhancementLevel:SetColor(ItemThemes.COLOR_GOOD)
        else
