...
local PROJECTILE_SOURCES = script:GetChildren()
local currentSource = 1
...
function Attack(target)	
	...
	if #PROJECTILE_SOURCES > 0 then
		startPos = PROJECTILE_SOURCES[currentSource]:GetWorldPosition()
		rotation = PROJECTILE_SOURCES[currentSource]:GetWorldRotation()
		direction = rotation * Vector3.FORWARD
		print(tostring(startPos))
		currentSource = currentSource + 1		
		if currentSource > #PROJECTILE_SOURCES then currentSource = 1 end
	end
	
	if not IS_PROJECTILE_HOMING then
		print(tostring(currentSource))
		print(tostring(startPos))
