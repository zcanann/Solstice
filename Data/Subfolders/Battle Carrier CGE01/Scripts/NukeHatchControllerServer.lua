...
function GetDoorOffset()
	local result = -DOOR_ROOT:GetPosition().x / OPEN_DISTANCE
...
function SetCurrentOffset(offset)
	...
	DOOR_ROOT:SetPosition(Vector3.New( -OPEN_DISTANCE * offset, 0.0,0.0))
end
...
function SetTargetOffset(offset)
	targetDoorOffset = offset
	DOOR_ROOT:MoveTo(Vector3.New( -OPEN_DISTANCE * offset, 0.0,0.0), OPEN_DISTANCE * math.abs(targetDoorOffset - GetDoorOffset()) / SPEED, true)
end
